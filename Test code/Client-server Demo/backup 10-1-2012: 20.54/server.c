			...
			for(j = 0; j < IP_ADDRESSES; j++) {
				if(ip_addresses[j] == '\0') {
					...
					while(last_block <= total) {
						...
						rv = write(p, message_buffer, BUFFSIZE);
						if (rv < 0) {
							perror("Error sending");
							//exit(EXIT_FAILURE);
						}
						...
						/*  TEST */
						char *received;
						received = malloc(BLOCK_SIZE+1);
						int bytesRead = 0;
					    while(bytesRead < BLOCK_SIZE) {
					    	printf("WHILE\n");
							int ret = read(p, received, BLOCK_SIZE);
							if(ret <= 0) {
								printf("STOP");
								break;
						   	}
					    ...
					    printf("AFTER NEW READ:  %d\n", bytesRead);
						nr_bytes_recv = bytesRead;
						printf("-D: after recv bytes\n");
						if (nr_bytes_recv == 0 || nr_bytes_recv > BLOCK_SIZE ) {
							perror("Buffer too small");
							exit(EXIT_FAILURE);
						}
						printf("-D: end + nr:%d\n", nr_bytes_recv);
						printf("-O: before message\n");
						char *string1 = malloc(BUFFSIZE);
						string1[0] = '\0';
						sprintf(string1, "POST|%s|%s|%d|%s\n", start_parameters[i].filename, start_parameters[i].filesize, last_block, data_block);
						...
						printf("-O: before send rv: %d\n", rv);
						rv = write(ss, string1, string1Length);
						free(string1);

						printf("-O: after send rv: %d\n", rv);
						if (rv < 0) {
							perror("Error sending");
							exit(EXIT_FAILURE);
						}
						...
						// receive data in buffer
						printf("-O: free....\n");
						char *receivedUploadReply = malloc(BUFFSIZE);
						//sleep(2);	
						nr_bytes_recv = read(ss, receivedUploadReply, BUFFSIZE);
						//received[nr_bytes_recv]= '\0'; // close string
						printf("-O: after recv\n");
						...
						while(strncmp(receivedUploadReply, "OK\n", 2) != 0) {		
						}
						...
						//down_info[thread_nr].block_nr = blockNr;
						free(receivedUploadReply);
						//free(sendBuffer);
						/* TEST */

						/*
						// Wait until block is received
						nr_bytes_recv = read(p, data_block, BLOCK_SIZE + 1);
						if (nr_bytes_recv > BLOCK_SIZE) {
							perror("Buffer too small");
							//exit(EXIT_FAILURE);
						}
						...
						rv = write(ss, store_buffer, BLOCK_SIZE + 50);
