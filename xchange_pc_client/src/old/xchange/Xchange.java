...
public class Xchange {
	...
	public int getLocalBlock(String fn, int block_nr, byte[] b)
			throws Exception {
		...
		if (file.exists()) {
			...
			if (tmpFile.exists()) {
				...
				if (t == -1) {
					System.err
							.println("ERROR : reached EOF in file xchange/INFO"
									+ fn + ".!info");
				}
				...
				if ((char) t == '1') {
					...
					System.err
							.println("ERROR : block not present says info file xchange/INFO/"
									+ fn + ".!info");
					return 0;
				}
			} else {
				System.err.println("ERROR : file xchange/shared/" + fn
						+ " not found");
				return 0;
			}
		}
	...
	/**
	 * make a list of all the shared files and their sizes in xchange/shared in
	 * a reply to a SEARCH from another peer this list is excluding files with
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (pattern.equals("*") || (tmpName.indexOf(pattern) != -1)) {
				if (tmpName.indexOf("!xch") == -1) {
					al.add(new Fileinfo(this, files[i].getName(), files[i]
							.length(), ip));
				}
			}
		}
	...
	/**
	 * make a list of all the shared files on the peer with specified ip address
	...
	public ArrayList<Fileinfo> searchFile(String pattern, String ip) {
		...
		if (line.startsWith("FAIL")) {
			System.err.println("ERROR : Peer " + ip
					+ " returned FAIL on SEARCH request");
			return al;
		} else {
			...
			if (!line.equals("")) {
				...
				for (int i = 0; i < s.length; i = i + 2) {
					al.add(new Fileinfo(this, s[i], Long.parseLong(s[i + 1]),
							ip));
				}
			}
			return al;
