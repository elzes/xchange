...
public class XStorageServer {
	...
	/**
	 * after router download is stopped, transfer the downloaded blocks from the
	 * storage server (GET)
	 */
	...
	public void TransferBlocks(String filename, int block_nr) {
		try {
			...
			if (line.startsWith("OK")) {
				...
				FileHelper.writeBlockToFile(new File(filename), block_nr,
						FileHelper.BLOCK_SIZE, bytes);
			} else {
				System.err
						.println("ERROR : Storageserver returned FAIL on GET request");
			}
	...
	public void removeFile(String fileName) {
		...
		if (line.equals("FAIL")) {
			System.err
					.println("ERROR : Storageserver returned FAIL on ADD request");
		}
	}
}
