...
public class Download extends Thread {
	...
	public Download(GUI gui, Fileinfo fileinfo) {
		...
		if (Debug.DEBUG) {
			System.out.println("New download of file " + fileinfo.name
					+ " blocks= " + fileinfo.nr_of_blocks + " ip= "
					+ fileinfo.ip);
		}
		try {
			preAllocateFiles();
	...
	/*
	 * at start of a new download, disk space is pre-allocated check is file
	 * exists, else create a new file of given size in xchange/shared during
	 * download the file has exentsion .!xch, until file is complete in
	 * addition, create info file in xchange/INFO with extension .!info
	 * 
	 * in the info file each block is represented by a byte if we use the chars
	 * we can view/edit the info file as text file :-) '0' = block not avaiable,
	 * '1' = block available
	 */
	public void preAllocateFiles() throws IOException {
		File file = FileHelper.loadFile(FILE_LOCATION + fileinfo.name
				+ FileHelper.TMP_EXT);
		if (file.exists()) {
			System.err.println("NOTIFY : file " + fileinfo.name
					+ " found pre-allocated");
			return;
		} else {
			// create a new file
			...
			FileHelper.createRandomAccesFileFromFile(new File(
					FILE_INFO_LOCATION + fileinfo.name + FileHelper.INFO_EXT),
					fileinfo.nr_of_blocks);

			if (Debug.DEBUG) {
				System.out.println("Created new download file : "
						+ FILE_LOCATION + fileinfo.name + FileHelper.TMP_EXT);
				System.out.println("Created new info file : "
						+ FILE_INFO_LOCATION + fileinfo.name
						+ FileHelper.INFO_EXT);
			}
		}
	}
	...
	void getRemoteBlock(String fileName, int block_nr, String ip, int size)
		...
		BufferedInputStream bufferedInputStream = new BufferedInputStream(
				ps.getInputStream(), gui.xchange.BLOCKSIZE);

		// example : "GET hello.mp3 312\n"
		printWriter.println("GET " + fileName + " " + block_nr);

		/**
		 * try to read size bytes note : n should be the number of bytes
		 * actually read, but testing shows that n always equals BLOCKSIZE ...
		 */
		...
		if (Debug.DEBUG) {
			System.out.println("Received block with size " + size
	...
	public void run() {
		...
		for (int i = 0; i < fileinfo.nr_of_blocks; i++) {
			...
			if (gui.stop_all_downloads) {
				if (Debug.DEBUG) {
					System.out
							.println("Stop download of file " + fileinfo.name);
				}
				return;
			}
			...
			try {
				...
				if (!file.exists()) {
					System.err
							.println("ERROR : file " + FILE_LOCATION
									+ fileinfo.name + FileHelper.TMP_EXT
									+ " not found");
					return;
				}
				FileHelper.writeByteArrayToFile(file,
						i * gui.xchange.BLOCKSIZE, this.byteArray, size);

				// and update info file
				File infoFile = new File(FILE_INFO_LOCATION + fileinfo.name
						+ ".!info");
				if (!infoFile.exists()) {
					System.err.println("ERROR : file " + FILE_INFO_LOCATION
							+ fileinfo.name + ".!info not found");
					return;
				}
			...
			if (Debug.DEBUG) {
				System.out.println("Downloaded new block of file "
						+ FILE_LOCATION + fileinfo.name);
			}
		...
		FileHelper.renameTmpFile(FILE_LOCATION + fileinfo.name
				+ FileHelper.TMP_EXT);

		if (Debug.DEBUG) {
			System.out.println("Removed extension from file : " + FILE_LOCATION
					+ fileinfo.name + ".!xch");
		}
	}
}
